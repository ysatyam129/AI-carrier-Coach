...
export default function Register({ setUser }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { user } = await authAPI.register(formData.name, formData.email, formData.password)
      setUser(user)
        ...
        <Card className="border-0 shadow-2xl bg-white/80 backdrop-blur-xl">
          <CardHeader className="text-center pb-6 pt-8">
            ...
            <CardDescription className="text-slate-600 text-lg mt-2">
              Join AI Career Coach and accelerate your career
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="px-8 pb-8">
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {formData.password && (
                  <div className="flex items-center space-x-2 mt-2">
                    <div className="flex-1 bg-slate-200 rounded-full h-2">
                      <div className={`h-2 rounded-full transition-all duration-300 ${strength.color}`} 
                           style={{ width: strength.strength === 'weak' ? '33%' : strength.strength === 'medium' ? '66%' : '100%' }}></div>
                    </div>
              ...
              <Button 
                type="submit" 
                className="w-full h-12 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 border-0 rounded-xl font-semibold text-white shadow-lg hover:shadow-xl transition-all duration-300 group mt-6" 
                disabled={loading}
              >
                {loading ? (
